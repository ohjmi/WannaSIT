...
async function createPost(req, res) {
  ...
  try {
    ...
    res.status(500).json({ message: "서버 에러: 게시글 등록 실패" });
  } finally {
...
async function updatePost(req, res) {
  ...
  if (!authorId) {
    res.status(404).json({ message: "게시글 없음: 게시글 수정 실패"});
    return;
  }

  // 본인이 작성하지 않은 게시글을 수정하려는 경우
  if (req.sessionID !== authorId) {
    res.status(403).json({ message: "권한 없음: 게시글 수정 실패" });
    return;
  }
  ...
  try {
    ...
    res.status(500).json({ message: "서버 에러: 게시글 수정 실패" });
  } finally {
    endConnection(connection);
  }
}
...
async function deletePost(req, res) {
  ...
  // boardID에 해당하는 게시글이 존재하지 않는 경우
  if (!authorId) {
    res.status(404).json({ message: "게시글 없음: 게시글 삭제 실패"});
    return;
  }

  // 본인이 작성하지 않은 게시글을 삭제하려는 경우
  if (req.sessionID !== authorId) {
    res.status(403).json({ message: "권한 없음: 게시글 삭제 실패" });
    return;
  }
  ...
  try {
    ...
    res.status(500).json({ message: "서버 에러: 게시글 삭제 실패" });
  } finally {
    endConnection(connection);
  }
}
