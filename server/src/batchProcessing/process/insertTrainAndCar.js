    ...
    try {
      ...
      for (let train of station.train) {
        const [trainId] = await connection.execute(
          ...
          VALUES (?, ?, ?, ?, ?, ?)`,
          [getKeyByValue(stationList, station.stationName), train.updnLine, train.dow, train.hh, train.mm, train.getOffCount]
        );

        for (let i = 0; i < 10; i++) {
          await connection.execute(
            `
          INSERT INTO car (train_id, estimated_count) VALUES (?, ?)`,
            [trainId.insertId, train.estimatedCount[i]]
          );
        }
      }
